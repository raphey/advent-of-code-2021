...
def part_1(raw_input):
    ...
    def get_successors(state):
        ...
        (_, cost), locs = state
        for k in range(4):
            other_rooms = ''.join('ABCD'[q] for q in range(4) if q != k)
            step_cost = 10**k
            for z, (i0, j0) in enumerate(locs[k]):
                ...
                if in_hallway(i0, j0):
                    ...
                    if not is_occupied(locs, 2, target_j) and not is_occupied(locs, 1, target_j) and not any(is_occupied(locs, 1, jj) for jj in range(start_j + 1, target_j)):
                        if is_occupied(locs, 3, target_j) and (3, target_j) not in locs[k]:
                            # can't block someone in
                            continue
                        new_locs = list(locs)
                ...
                if in_any_room(i0, j0):
                    if i0 == 2:  # only allow moving out to hall from top room position
                ...
                for di, dj in (-1, 0), (1, 0):   # only moving up and down
                    new_i, new_j = i0 + di, j0 + dj
                    if is_wall(new_i, new_j):
                        continue
                    if is_occupied(locs, new_i, new_j):
                        continue
                    if in_any_room(new_i, new_j) and not in_any_room(i0, j0):
                        continue   # already handled moving from hallway into room
                    if in_any_room(i0, j0) and not in_any_room(new_i, new_j):
                        continue   # already handled moving from room into hallway
                    # this should just be fiddling up and down within rooms
                    new_locs = list(locs)
                    new_locs[k] = ((new_i, new_j), (i_other, j_other))
